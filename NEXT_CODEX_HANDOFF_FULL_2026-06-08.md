# Freshon Admin 전체 인수인계 메모 - 2026-06-08

## 사용자가 하는 업무와 이 도구를 만드는 이유

사용자는 Freshon/물류 운영 현장에서 배차, 권역, 상차 리포트, 신규 권역 판단을 실제 업무 기준으로 관리하고 있다. 이 프로젝트는 단순히 지도를 예쁘게 보는 앱이 아니라, 매일 반복되는 배차 판단과 엑셀 확인, 카톡 보고, 호차 조정 업무를 줄이기 위한 실무 도구다.

현재 사용자의 주요 업무는 네 가지다.

1. 고정배차/권역 관리
   - 월별 배차 엑셀, 고객사 정보, 매출/주소 데이터를 보고 각 호차가 어느 권역을 맡는지 확인한다.
   - 고객코드, 고객명, 주소로 기존 고객이 어느 호차에 붙어 있는지 빠르게 찾아야 한다.
   - 지도는 권역과 동선을 이해하기 위한 수단이고, 실제 업무에서는 검색과 표 확인이 매우 중요하다.

2. WMS 배차 검토
   - Freshon WMS에서 배차계획조정/배차현황 파일을 내려받아 호차별 금액, 톤수, 착지 수를 확인한다.
   - 금액 기준으로 초과 차량을 찾고, 기존 호차끼리 물량을 붙이는 것이 아니라 튀는 지역구나 특정 착지 묶음을 새 용차로 빼는 판단을 한다.
   - 단순히 “101호 초과”라고 표시하는 것보다, 왜 초과인지, 어떤 착지를 빼면 좋은지, 금액과 동선 근거가 무엇인지 보여주는 것이 중요하다.

3. 일일동선 확인
   - 특정 날짜/호차가 실제로 어디를 배송했는지 보고 싶어 한다.
   - 기존 업로드/캐시 방식은 2건만 뜨거나 오류가 반복되어 신뢰가 낮았다.
   - 최종 방향은 Freshon 직접 조회 기반이다.
   - Freshon에서 물류센터, 입고요청일, 배차구분 야간배송, 호차를 넣고 조회한 결과를 가져와 운영지도 주소 데이터와 매칭해 동선을 표시하는 흐름을 원한다.

4. 모바일 데일리 상차 리포트
   - 현장에서 전표 사진과 미상차 사진을 정리하고 카톡으로 보낼 문구를 빠르게 만들기 위한 모바일/PWA 기능이다.
   - 전표 OCR로 매장명을 읽고 고객 마스터에서 고객코드, 매장명, 호차, 착순을 찾아야 한다.
   - 적재함 사진은 호차당 1장이고, 전표와 미상차 사진은 한 세트로 관리한다.
   - 전체 기록은 별도 페이지에서 검색 가능해야 한다.

이 프로젝트의 진짜 목적:

- 엑셀을 눈으로 뒤지고 복붙하는 시간을 줄인다.
- 호차별 금액/톤수/권역 판단을 자동화한다.
- 신규 주소가 기존 권역에 들어갈 수 있는지 빠르게 판단한다.
- 과배차 차량에서 용차로 뺄 후보를 근거 있게 추천한다.
- 현장 사진/전표/미상차 보고를 모바일에서 정리한다.
- Freshon, WMS, 딜리버리 어드민, 사수가 만든 지도에서 각각 좋은 점을 뽑아 하나의 실무 도구로 합친다.
...
- UI가 느리거나 복잡하면 안 된다.
- 결과는 설명 가능해야 한다.
- 고객코드, 고객명, 주소 중 하나라도 있으면 검색되어야 한다.
- 좌표가 없다고 검색이 안 되는 것은 안 된다.
- 좌표는 지도 표시용이고, 검색은 주소 문자열만으로도 되어야 한다.
- 예쁜 대시보드보다 일할 때 손이 덜 가는 화면이 더 중요하다.
...
사용자는 이 프로젝트에서 운영지도, WMS 배차 검토, 일일동선, 모바일 데일리 상차 리포트를 같이 발전시키고 있다. 다음 Codex는 기존 UI를 그대로 답습하지 말고, 새 UI 작업 전에는 먼저 시안을 보여주는 방식이 좋다.
...
아래 파일들은 이미 freshon-admin 프로젝트에 반영되어 있다. 삭제하지 말 것.
...
이 이미지는 최종 UI 방향 참고용이다. 단, 사용자는 “시안이 기대보다 너무 안 바뀐다”고 여러 번 말했으니, 이미지를 그대로 베끼기보다 기능 흐름을 유지하면서 더 실무적인 모바일 앱 UI로 다시 짜는 것이 좋다.
...
- Freshon 경로:
  ...
  - 호차 검색
  - 조회
- 조회 결과에서 톤수, 주문금액, 고객 목록을 가져와 운영지도 주소 데이터와 매칭해서 일일동선을 만들고 싶어함
