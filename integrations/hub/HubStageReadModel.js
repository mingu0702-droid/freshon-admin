...
function hubStageModelHash_(value){return Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256,JSON.stringify(value),Utilities.Charset.UTF_8).map(function(n){return('0'+(n&255).toString(16)).slice(-2);}).join('');}
// Compatibility ONLY: existing source cursor fingerprints are not transport
// signatures. Keep their persisted contract unchanged during SEND migration.
function hubStageModelLegacyHash_(value){return Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256,JSON.stringify(value),Utilities.Charset.US_ASCII).map(function(n){return('0'+(n&255).toString(16)).slice(-2);}).join('');}
function hubStageModelSignature_(text,secret){return Utilities.computeHmacSha256Signature(text,secret,Utilities.Charset.UTF_8).map(function(n){return('0'+(n&255).toString(16)).slice(-2);}).join('');}
// Pure inspection and planning only. No Drive writes, checkpoint save, trigger,
// transport or resume calls. The returned candidate must NOT be auto-applied.
function hubStageModelUtf8InspectShard_(key,entry,text){
  let rows;try{rows=JSON.parse(text);}catch(ignored){throw new Error('MODEL_MIGRATION_JSON');}
  if(!Array.isArray(rows)||rows.length!==entry.count)throw new Error('MODEL_MIGRATION_COUNT');
  const legacyHash=hubStageModelLegacyHash_(rows),utf8Hash=hubStageModelHash_(rows);
  if(entry.hash!==legacyHash&&entry.hash!==utf8Hash)throw new Error('MODEL_MIGRATION_HASH');
  return{key:key,id:entry.id,count:rows.length,previousHash:entry.hash,legacyHash:legacyHash,utf8Hash:utf8Hash};
}
function hubStageModelUtf8MigrationPlan_(state,reports){
  if(!state||state.phase!=='ERROR'||state.sendAt!==0||!Array.isArray(state.sources)||state.source!==state.sources.length||state.sources.length!==4
    ||!Number.isSafeInteger(state.generation)||state.generation<1||!Number.isSafeInteger(state.scanned)||state.scanned<0
    ||state.sources.some(function(s){return !Number.isInteger(s.end)||s.next!==s.end+1;}))throw new Error('MODEL_MIGRATION_STATE');
  const keys=Object.keys(state.shards),seen={};
  if(!keys.length||!Array.isArray(reports)||reports.length!==keys.length)throw new Error('MODEL_MIGRATION_INCOMPLETE');
  reports.forEach(function(r){
    const e=state.shards[r.key];
    if(!e||seen[r.key]||r.id!==e.id||r.count!==e.count||r.previousHash!==e.hash
      ||!/^[a-f0-9]{64}$/.test(r.utf8Hash)||!/^[a-f0-9]{64}$/.test(r.legacyHash)
      ||(e.hash!==r.legacyHash&&e.hash!==r.utf8Hash))throw new Error('MODEL_MIGRATION_MANIFEST');
    seen[r.key]=r;
  });
  const candidate=JSON.parse(JSON.stringify(state));
  keys.forEach(function(key){candidate.shards[key].hash=seen[key].utf8Hash;});
  return{dryRun:true,shards:keys.length,legacyMatches:reports.filter(function(r){return r.previousHash===r.legacyHash;}).length,
    utf8Calculated:reports.length,changedHashes:reports.filter(function(r){return r.previousHash!==r.utf8Hash;}).length,candidate:candidate};
}
...
function hubStageModelSave_(s,options){if(!options||options.preserveUpdatedAt!==true)s.updatedAt=new Date().toISOString();const f=DriveApp.getFileById(s.stateId),text=JSON.stringify(s);f.setContent(text);if(f.getBlob().getDataAsString()!==text)throw new Error('MODEL_CHECKPOINT_VERIFY');}
...
function hubStageModelSend_(message,target){
  const destination=hubReadModelTarget_(target||'stage');
  const body={timestamp:String(Date.now()),nonce:Utilities.getUuid().replace(/-/g,''),message:message},text=JSON.stringify(body);
  const secret=PropertiesService.getScriptProperties().getProperty(destination.secretProperty);
  if(!secret||secret.length<32)throw new Error('MODEL_AUTH_MISSING');
  const signature=hubStageModelSignature_(text,secret);
  const r=UrlFetchApp.fetch(destination.endpoint,{method:'post',contentType:'application/json',payload:text,headers:{'x-stage-model-signature':signature},muteHttpExceptions:true,followRedirects:false});
  const status=r.getResponseCode();
  if(status!==200){const error=new Error('MODEL_STAGE_HTTP_'+status);error.modelAuthFatal=status===401||status===403;throw error;}
  // ACK only; never log response contents or protected lookup values.
  return status;
}
function hubStageModelAuthHalted_(s){return !!s&&s.authHaltGeneration===s.generation;}
function hubStageModelOneTime_(s){return !!s&&!!s.oneTimeRecovery;}
function hubStageModelRequest_(params){
  hubMapHttpValidateOnlyKeys_(params,['target']);
  const target=params.target===undefined?'stage':params.target;
  hubReadModelTarget_(target);
  ...
  try{
    ...
    // RESTORE has a separate cursor. Never rewind or mutate a published BUILD.
    if(s&&s.phase==='DONE'&&!hubStageModelAuthHalted_(s))return hubStageModelRestoreRequest_(s,target);
    // Production restore can only consume the approved published model. Never BUILD.
    if(target==='production')throw new Error('MODEL_RESTORE_NOT_PUBLISHED');
    // This generation has approval for ONE recovery only. A Stage cold start
    // must not reset SEND, replay DONE, clear an auth stop or reinstall watchdog.
    if(s&&(s.generation===1789401132459||hubStageModelAuthHalted_(s)))
      return{data:{phase:s.phase,scanned:s.scanned,continuation:s.phase==='SEND'?'EXISTING_ONLY':'NONE'},cached:false};
...
function hubStageModelBuildBatch_(s){
  ...
  const headers=sheet.getRange(1,1,1,width).getValues()[0].map(String),h=hubStageModelLegacyHash_(headers);
  ...
  if(source.tail){const actual=sheet.getRange(source.next-1,1,1,Math.min(width,5)).getDisplayValues()[0];if(hubStageModelLegacyHash_(actual)!==source.tail)throw new Error('MODEL_SOURCE_MOVED');}
  ...
  source.next+=count;source.tail=hubStageModelLegacyHash_(tail);s.scanned+=count;
}
function hubStageReadModelContinue(){
  ...
  try{
    ...
    s=hubStageModelLoad_();if(!s||hubStageModelAuthHalted_(s)||s.phase==='ERROR'||s.phase==='DONE')return;
    if(hubStageModelOneTime_(s))hubStageModelRecoveryAssert_(s);
    ...
    while(Date.now()<deadline){
      ...
      if(s.phase==='SEND'){
        ...
        if(hubStageModelOneTime_(s))hubStageModelRecoveryAssert_(s);
        if(s.sendAt===0){const http=hubStageModelSend_({type:'begin',generation:s.generation});if(hubStageModelOneTime_(s)){s.oneTimeRecovery.firstRequestHttp=http;hubStageModelSave_(s);}}
        if(s.sendAt<keys.length){
          const key=keys[s.sendAt],e=s.shards[key],rows=JSON.parse(DriveApp.getFileById(e.id).getBlob().getDataAsString());
          if(hubStageModelOneTime_(s)&&(rows.length!==e.count||hubStageModelHash_(rows)!==e.hash))throw new Error('MODEL_RECOVERY_SHARD_CHANGED');
          const http=hubStageModelSend_({type:'shard',generation:s.generation,kind:key.split(':')[0],date:key.split(':')[1],rows:rows,hash:e.hash});
          if(hubStageModelOneTime_(s))hubStageModelRecoveryAck_(s,key,rows,http);
          s.sendAt++;hubStageModelSave_(s);continue;
        }
        if(hubStageModelOneTime_(s)){hubStageModelRecoveryCommitCheck_(s);hubStageModelSave_(s);}
        const commitHttp=hubStageModelSend_({type:'commit',generation:s.generation,startDate:s.startDate,endDate:s.endDate,manifest:keys.map(function(k){return{key:k,hash:s.shards[k].hash,count:s.shards[k].count};})});
        if(hubStageModelOneTime_(s))s.oneTimeRecovery.commitHttp=commitHttp;
        s.phase='DONE';s.errors=0;hubStageModelSave_(s);break;
      }
    }
  }catch(e){
    if(s){
      const code=/^MODEL_[A-Z_0-9]+$/.test(String(e.message))?e.message:'MODEL_OPERATION_FAILED',fatalAuth=e.modelAuthFatal===true||/^MODEL_STAGE_HTTP_(401|403)$/.test(code);
      s.errors=s.lastError===code?(s.errors||0)+1:1;s.lastError=code;
      if(fatalAuth){s.authHaltGeneration=s.generation;s.authHaltHttp=Number(code.slice(-3));s.authHaltAt=new Date().toISOString();}
      if(fatalAuth||hubStageModelOneTime_(s)||s.errors>=3||code==='MODEL_TASK_CONFLICT'||code==='MODEL_SOURCE_MOVED')s.phase='ERROR';
      hubStageModelSave_(s);
    }
    console.warn('Stage read model maintenance failed; inspect safe checkpoint status.');
  }finally{
    if(s&&(hubStageModelAuthHalted_(s)||s.phase==='DONE'||s.phase==='ERROR'))ScriptApp.getProjectTriggers().filter(function(t){return t.getHandlerFunction()===HUB_STAGE_MODEL.continuation;}).forEach(function(t){ScriptApp.deleteTrigger(t);});
    else if(s)hubStageModelSchedule_(true);
    if(s&&s.phase==='ERROR'&&!hubStageModelAuthHalted_(s)&&!hubStageModelOneTime_(s))try{hubStageModelSend_({type:'status',phase:'ERROR',scanned:s.scanned,total:0});}catch(ignored){}
    lock.releaseLock();
  }
}
