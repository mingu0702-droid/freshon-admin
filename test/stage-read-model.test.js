...
import {createStageReadModel,stageReadModelPending} from '../src/stageReadModel.js';
...
test('pending bootstrap keeps UI automatic wait; genuine build error remains failure',()=>{const f=fixture(),e=Error('READ_MODEL_NOT_READY');for(const phase of ['WAITING','BUILD','HYDRATING']){f.send({type:'status',phase});const r=stageReadModelPending(f.model,e);assert.equal(r.status,202);assert.equal(r.body.error,null);assert.equal(r.body.meta.complete,false);}f.send({type:'status',phase:'ERROR'});assert.equal(stageReadModelPending(f.model,e).status,503);});
