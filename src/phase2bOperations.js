...
export function normalizePhase2bDetail(data, dispatch = {}) {
  ...
  return {
    ...
    lat: (data?.lat ?? data?.latitude) != null && (data?.lat ?? data?.latitude) !== "" && Number.isFinite(Number(data?.lat ?? data?.latitude)) ? Number(data.lat ?? data.latitude) : null,
    lng: (data?.lng ?? data?.longitude) != null && (data?.lng ?? data?.longitude) !== "" && Number.isFinite(Number(data?.lng ?? data?.longitude)) ? Number(data.lng ?? data.longitude) : null,
