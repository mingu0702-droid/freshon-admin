// Parsing helpers only: raw sources are not safe response objects.
export function sanitizeLocationMessage(value, centerMemo = "") {
  ...
  return text.split(/\n|[;；]+|\/(?=[ \t]*(?:[*※•]|\(|배송위치|출입방법|보관위치|특이사항|배송요일|Claim|클레임|센터))|\/[ \t]+(?=[가-힣\[])/iu)
...
export function parseLocationMessage(value) {
  ...
  for (let line of lines) {
    ...
    }else if(/^(?:특이사항|보관위치|냉장보관|냉동보관|실온보관)\s*[:：]/u.test(line)){
  ...
  // Unlabelled/ambiguous prose must not be a browser-visible fallback.
  fields.rawMemo="";
  return fields;
}
